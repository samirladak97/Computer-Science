...
public class WordTree
{
	...
	public void insert(String word)
	{
		...
		for (i=0; i<word.length(); i++)
		{
			...
			if (temp.children[(int)a] == null)
			{
				temp.createChild(a);
				temp = temp.getChild(a);
			}
			else
			{
				temp = temp.getChild(a);
	...
	//create a global ArrayList<String> for storage when using helper depthfirst method
	//depthfirst is a recursive postorder helper method to get all the words in the tree with the given prefix
	//getListPrefixMatches uses depthfirst to get all the words with the given prefix
	
	public ArrayList<String> matches = new ArrayList<String>();
	
	public void depthfirst(WordTreeNode start)
	{
		int size = 0;
		int i;
		for (i=0; i<256; i++)
		...
		if (size > 0)
		{
			...
			for (j=0; j<256; j++)
			{
				...
				{
					depthfirst(start.children[j]);
				}
			}	
		}
		if (start.isEndOfWord())
		{
			matches.add(start.toString());
		}
	...
	public ArrayList<String> getListPrefixMatches( String prefix )
	{
		...
		{
			char a = prefix.charAt(x);
			if (tmp.getChild(a) != null)
			{
				tmp = tmp.getChild(a);
			}
			else
